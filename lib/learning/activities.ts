...
export function topicAction(progress?: TopicProgress, evidence: LearningEvidence[] = [], hasActivities = true) {
  if (!progress?.completionScore) return { label: "Study", reason: "Trabaja el contenido y guarda evidencia de Study.", anchor: "study" };
  if (!hasActivities) return { label: "Continuar el tema", reason: "Revisa el alcance y tus notas; el desarrollo técnico y las actividades esperan material académico autorizado.", anchor: "study" };
