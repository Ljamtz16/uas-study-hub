...
describe("V0.3 knowledge foundation",()=>{
 it("KG-02 every graph Topic resolves",()=>{expect(validateGraph()).toEqual([]);expect(graphNodes.filter(n=>n.kind==="topic")).toHaveLength(19);for(const n of graphNodes)expect(topics.some(t=>t.id===n.topicId)).toBe(true);});
 ...
 it("each academic page has current provenance, valid concepts and adapted objectives",()=>{for(const record of academicRecords){expect(record.source.academic_year).toBe("2026/2027");expect(record.source.validation_status).toBe("official_current");expect(record.objective_kind).toBe("pedagogical_adaptation");for(const id of record.concept_ids)expect(concepts.some(c=>c.id===id)).toBe(true);const mdx=matter(fs.readFileSync(record.content_ref,"utf8"));expect(mdx.data.stable_id).toBe(record.stable_id);expect(mdx.content).toContain("Desarrollo técnico pendiente");}});
});
