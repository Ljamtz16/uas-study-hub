...
for(const offline of [false,true])test(`V0.2 learning loop, reload and recommendations ${offline?"offline":"online"}`,async({page,context})=>{
  await page.goto("/");
  await page.getByRole("link",{name:"Cursos"}).click();await page.getByRole("link",{name:"Ver curso"}).click();await page.goto("/study/fund-radio-links-intro");
  ...
  await page.getByRole("link",{name:"Inicio"}).click();await expect(page.getByText("Study: Trabaja el contenido y guarda evidencia de Study.")).toBeVisible();await expect(page.getByTestId("course-progress")).toHaveText("0%");
});
